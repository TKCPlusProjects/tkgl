...
ShapeCircle::ShapeCircle(Point center, Point point, bool is_solid) : Shape(TypeCircle), center(center), point(point), is_solid(is_solid) {
  CalRadius();
  CalVector();
}
...
void ShapeCircle::CalPoint() {
  point = vector.Length() ? (center + radius * vector) : Point(center.x + radius, center.y);
}
